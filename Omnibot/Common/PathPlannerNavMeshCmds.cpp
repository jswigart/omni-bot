...
void PathPlannerNavMesh::cmdNavSave( const StringVector& args )
{
...
void PathPlannerNavMesh::cmdNavLoad( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
...
void PathPlannerNavMesh::cmdNavView( const StringVector& args )
{
	const char *strUsage [] =
	{
	...
	mFlags.mViewMode = ( uint32_t )viewmode;
}

void PathPlannerNavMesh::cmdNavShowObstacles( const StringVector& args )
{
	const char *strUsage [] =
	{
	...
	mFlags.mViewObstacles = bEnable ? 1 : 0;
}

void PathPlannerNavMesh::cmdNavDisableObstacles( const StringVector& args )
{
	const char *strUsage [] =
	{
	...
	mFlags.mDisableObstacles = bEnable;
...
void PathPlannerNavMesh::cmdNavViewConnections( const StringVector& args )
{
	const char *strUsage [] =
	{
	...
	mFlags.mViewConnections = bEnable ? 1 : 0;
}

void PathPlannerNavMesh::cmdNavViewFlags( const StringVector& args )
{
	const char *strUsage [] =
	{
	...
	mFlags.mViewFlags = bEnable ? 1 : 0;
}

void PathPlannerNavMesh::cmdNavEnableStep( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
	...
	mFlags.mStepProcess = bEnable ? 1 : 0;
}

void PathPlannerNavMesh::cmdNavStep( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
...
void PathPlannerNavMesh::cmdNavStats( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
	...
	EngineFuncs::ConsoleMessage( va( "%d Sectors", mNavSectors.size() ).c_str() );
	EngineFuncs::ConsoleMessage( va( "%d Total Sectors(with mirrored)", mRuntimeSectors.size() ).c_str() );
	EngineFuncs::ConsoleMessage( va( "%d Portals", numPortals ).c_str() );
	EngineFuncs::ConsoleMessage( va( "%d Tris in Static NavMesh Collision", mSectorCollision.NumTriangles() ).c_str() );
	EngineFuncs::ConsoleMessage( va( "%d Tris in Dynamic NavMesh Collision", dynTris ).c_str() );

	EngineFuncs::ConsoleMessage( "-= Obstacles =-" );
	EngineFuncs::ConsoleMessage( va( "%d Obstacles", mObstacles.size() ).c_str() );
	...
	EngineFuncs::ConsoleMessage( va( "Bounds Min( %.4f, %.4f, %.4f )", mapbounds.mMins[ 0 ], mapbounds.mMins[ 1 ], mapbounds.mMins[ 2 ] ).c_str() );
	EngineFuncs::ConsoleMessage( va( "Bounds Max( %.4f, %.4f, %.4f )", mapbounds.mMaxs[ 0 ], mapbounds.mMaxs[ 1 ], mapbounds.mMaxs[ 2 ] ).c_str() );
}

void PathPlannerNavMesh::cmdAutoBuildFeatures( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
...
void PathPlannerNavMesh::cmdTracePoly( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
...
void PathPlannerNavMesh::cmdUndo( const StringVector& args )
{
	if ( mCurrentTool )
...
void PathPlannerNavMesh::cmdCreatePlanePoly( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
...
void PathPlannerNavMesh::cmdCreateSlicePoly( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
...
void PathPlannerNavMesh::cmdCreateSliceSector( const StringVector& args )
{
	/*if(mFlags.mViewMode == 0)
	return;
...
void PathPlannerNavMesh::cmdEditSector( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
...
void PathPlannerNavMesh::cmdDupeSector( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
...
void PathPlannerNavMesh::cmdSplitSector( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
...
void PathPlannerNavMesh::cmdGroundSector( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
	...
	obTraceResult tr;
	for ( uint32_t i = 0; i < ns->GetNumVerts(); ++i )
	{
		...
		ns->mSector->SetVertZ( i, tr.mEndpos[ 2 ] );
	}

	InitSectors();
}

void PathPlannerNavMesh::cmdCommitPoly( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
...
void PathPlannerNavMesh::cmdDeleteSector( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
...
void PathPlannerNavMesh::cmdMirrorSectors( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
	...
	if ( args[ 1 ].find( "all" ) != std::string::npos )
		bAll = true;
...
void PathPlannerNavMesh::cmdSectorCreateConnections( const StringVector& args )
{
	Vector3f vPos;
	...
	EngineFuncs::ConsoleMessage( va( "Found %d portals for this sector( total %d )", ns->mPortals.size(), totalPortals ).c_str() );
}

void PathPlannerNavMesh::cmdSetMapCenter( const StringVector& args )
{
	if ( mFlags.mViewMode == 0 )
		return;
	...
	switch ( args.size() )
	{
		case 2:
		{
			if ( args[ 1 ] == "<position>" && Utils::GetLocalPosition( vMapCenter ) )
			{
			}
			else
			{
		...
		case 4:
		{
			if ( Utils::ConvertString( args[ 1 ], vMapCenter.X() ) &&
				Utils::ConvertString( args[ 2 ], vMapCenter.Y() ) &&
				Utils::ConvertString( args[ 3 ], vMapCenter.Z() ) )
			{
			}
			else
			{
...
void PathPlannerNavMesh::cmdObstacleAdd( const StringVector& args )
{
	if ( args.size() < 3 )
	{
	...
	const float len = (float)atof( args[ 1 ].c_str() );
	const float wid = (float)atof( args[ 2 ].c_str() );
	const float cost = (float)atof( args[ 3 ].c_str() );
...
void PathPlannerNavMesh::_BenchmarkPathFinder( const StringVector& args )
{
...
void PathPlannerNavMesh::_BenchmarkGetNavPoint( const StringVector& args )
{
	...
	//Vector3List sectorCenters;
	//sectorCenters.reserve( mRuntimeSectors.size() );
	//for(uint32_t i = 0; i < mRuntimeSectors.size(); ++i)
	//{
	//	sectorCenters.push_back( mRuntimeSectors[ i ].CalculateCenter() );
	//}
	...
	//{
	//	for(uint32_t s = 0; s < sectorCenters.size(); ++s)
	//	{
...
void PathPlannerNavMesh::cmdUpdateContents( const StringVector& args )
{
	for ( size_t i = 0; i < mRuntimeSectors.size(); ++i )
	{
...
void PathPlannerNavMesh::cmdSectorEdgeDrop( const StringVector& args )
{
...
void PathPlannerNavMesh::cmdPortalCreate( const StringVector& args )
{
...
void PathPlannerNavMesh::cmdPortalBlockCreate( const StringVector& args )
{
...
void PathPlannerNavMesh::cmdSetField( const StringVector& args )
{
	using namespace google;

	const bool both = args.at( 0 ).back() == 's';
	if ( both )
	{
		// easiest to just execute the command multiple times
		// than to adapt the function to be able to do both
		StringVector cmd = args;
		cmd[ 0 ] = "nav_setfield";
		cmdSetField( cmd );
		cmd[ 0 ] = "nav_setfieldm";
		cmdSetField( cmd );
	...
	const bool mirrored = args.at( 0 ).back() == 'm';
	...
	if ( args.size() <= 2 )
	{
		EngineFuncs::ConsoleError( va( "Usage: %s fieldname fieldvalue", args.at( 0 ).c_str() ).c_str() );
	...
	const std::string fieldName = Utils::StringToLower( args.at( 1 ).c_str() );
	...
	for ( size_t t = 0; t < tokens.size() && msgDesc != NULL; ++t )
	{
		...
		if ( fieldDesc == NULL )
		{
			EngineFuncs::ConsoleError( va( "Unknown field: '%s'", tokens[ t ].c_str() ).c_str() );
		...
		if ( fieldDesc->type() == protobuf::FieldDescriptorProto_Type_TYPE_MESSAGE )
		{
			...
			if ( msg == NULL )
			{
				EngineFuncs::ConsoleError( va( "Unknown field: '%s' in sub message '%s'",
					tokens[ t ].c_str(),
					fieldDesc->lowercase_name().c_str() ).c_str() );
				return;
			}
		}
	}

	const std::string fieldValue = args.at( 2 ).c_str();
	...
	if ( hidden )
	{
		EngineFuncs::ConsoleError( va( "Field: %s is hidden and cannot be set", args.at( 1 ).c_str() ).c_str() );
		return;
	}
	if ( settable )
	{
		EngineFuncs::ConsoleError( va( "Field: %s is not settable", args.at( 1 ).c_str() ).c_str() );
		return;
	}
	...
	try
	{
		switch ( fieldDesc->type() )
		{
			...
			{
				const protobuf::int64 val = stoll( args.at( 2 ).c_str() );
			...
			{
				const protobuf::uint64 val = stoul( args.at( 2 ).c_str() );
			...
			{
				const protobuf::int32 val = stoi( args.at( 2 ).c_str() );
				refl->SetInt32( msg, fieldDesc, val );
				break;
			}
			case protobuf::FieldDescriptorProto_Type_TYPE_BOOL:
			{
				const bool val = stoi( args.at( 2 ).c_str() ) != 0;
			...
			case protobuf::FieldDescriptorProto_Type_TYPE_GROUP:
				EngineFuncs::ConsoleError( va( "Can't set message field type 'Group'" ).c_str() );
				return;
			case protobuf::FieldDescriptorProto_Type_TYPE_MESSAGE:
				EngineFuncs::ConsoleError( va( "Can't set message field type 'Message', use dot(.) syntax to set child fields directly." ).c_str() );
				return;
			case protobuf::FieldDescriptorProto_Type_TYPE_BYTES:
				EngineFuncs::ConsoleError( va( "Can't set message field type 'bytes'" ).c_str() );
				return;
			case protobuf::FieldDescriptorProto_Type_TYPE_ENUM:
			{
				const protobuf::EnumValueDescriptor * enumVal = fieldDesc->enum_type()->FindValueByName( args.at( 2 ).c_str() );
				if ( enumVal != NULL )
				{
...
void PathPlannerNavMesh::cmdSaveToObjFile( const StringVector& args )
{
	//if(args.size() < 2)
	//{

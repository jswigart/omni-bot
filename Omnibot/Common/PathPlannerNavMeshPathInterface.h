...
private:
	...
	GoalList	mCurrentGoals;

	enum		{ MaxNodes=2048 };
	PlanNode	mNodes[MaxNodes];
	int			mUsedNodes;

#ifdef WIN32
	typedef std::hash_compare<int> HashMapCompare;
	typedef std::unordered_map<int, PlanNode*, HashMapCompare> NavHashMap;
#else

...
static void TPPLToVector3List( Vector3List & polyOut, const PathPlannerNavMesh::RuntimeNavSector & ns, const TPPLPoly & poly )
{
...
static void ClipperToVector3List( Vector3List & polyOut, const PathPlannerNavMesh::RuntimeNavSector & ns, const ClipperLib::Path & poly )
{
...
static void Vector3ListToClipper( ClipperLib::Path & polyOut, const Vector3List & poly )
{
	polyOut.resize( poly.size() );
	for ( size_t i = 0; i < poly.size(); ++i )
	{
		polyOut[ i ] = ClipperLib::IntPoint( ( ClipperLib::long64 )poly[ i ].X(), ( ClipperLib::long64 )poly[ i ].Y() );
	}
}

static void ClipperToTPPL( TPPLPoly & polyOut, const ClipperLib::Path & poly )
{
...
void AddPolyNodeToPaths( const ClipperLib::PolyNode& polynode, ClipperLib::NodeType nodetype, PolyList& polys )
{
	bool match = true;
	if ( nodetype == ClipperLib::ntClosed )
		match = !polynode.IsOpen();
	else if ( nodetype == ClipperLib::ntOpen )
		return;

	if ( !polynode.Contour.empty() && match )
	{
		polys.push_back( TPPLPoly() );
		ClipperToTPPL( polys.back(), polynode.Contour );
		
		bool isHole = polynode.IsHole();
		polys.back().SetHole( isHole );
		polys.back().SetOrientation( isHole ? TPPL_CW : TPPL_CCW );
	}
	for ( int i = 0; i < polynode.ChildCount(); ++i )
		AddPolyNodeToPaths( *polynode.Childs[ i ], nodetype, polys );
}

static void GenerateFromClipResults( const ClipperLib::PolyTree & polyTree,
	PathPlannerNavMesh::RuntimeNavSector & parent,
	const float cost,
	PathPlannerNavMesh::RuntimeSectorList & sectorsOut )
{
	PolyList inputPolys;
	AddPolyNodeToPaths( polyTree, ClipperLib::ntAny, inputPolys );
	
	// anticipate more polys than the input
	...
	if ( partition.ConvexPartition_HM( &inputPolys, &convexPolys ) != 0 )
	{
		...
		for ( PolyList::iterator it = convexPolys.begin(); it != convexPolys.end(); ++it )
		{
			...
			TPPLToVector3List( subSector.GetVertsMutable(), subSector, ply );
			sectorsOut.push_back( subSector );
		}
	}
	else
	{
		EngineFuncs::ConsoleError( va( "Sector %d error building subsectors", parent.mIndex ).c_str() );
	}
}
...
PathPlannerNavMesh::RuntimeNavSector::RuntimeNavSector( uint32_t index, NavSector * sector, NavmeshIO::SectorData * data )
	: mIndex( index )
	, mParent( NULL )
	, mSector( sector )
	, mSectorData( data )
...
bool PathPlannerNavMesh::RuntimeNavSector::InitSectorTransform()
{
	...
	if ( gEngineFuncs->IsMoverAt( center + Vector3f::UNIT_Z * 32.0f, center - Vector3f::UNIT_Z * 32.0f, entityMover ) )
	{
		bool good = true;
		...
		if ( good )
		{
			...
			mSectorData->mutable_mover()->set_id( entityMover.AsInt() );
...
void PathPlannerNavMesh::RuntimeNavSector::UpdateAutoFlags()
{
	...
	const int contents = gEngineFuncs->GetPointContents( center );
	...
	if ( mSectorData->inwater() )
	{
		...
		EngineFuncs::TraceLine( tr,
			Vector3f( tr.mEndpos ) + Vector3f( 0.f, 0.f, -1.f ),
			Vector3f( tr.mEndpos ) + Vector3f( 0.f, 0.f, -1024.f ),
			NULL, TR_MASK_WATER, -1, false );

		if ( tr.mFraction < 1.0f )
		{
			mSectorData->set_waterdepth( tr.mEndpos[ 2 ] - center.Z() );
		}
	}
}
...
{
	for ( SectorSet::iterator dit = mDropConnections.begin(); dit != mDropConnections.end(); ++dit )
	{
		RuntimeNavSector * otherSector = *dit;

		// see if the polygons actually overlap
		ClipperLib::Paths subjectPolys;
		subjectPolys.push_back( ClipperLib::Path() );
		Vector3ListToClipper( subjectPolys.back(), mPoly );
		...
		ClipperLib::Path dropPoly;
		for ( SectorSet::iterator it = dropSectors.begin(); it != dropSectors.end(); ++it )
		{
			dropPoly.resize( 0 );

			Vector3ListToClipper( dropPoly, ( *it )->mPoly );

			ClipperLib::Clipper clip;
			clip.AddPaths( subjectPolys, ClipperLib::ptSubject );
			clip.AddPath( dropPoly, ClipperLib::ptClip );

			PathPlannerNavMesh::RuntimeSectorList runtimedropSectors;

			ClipperLib::PolyTree solutionTree;
			if ( clip.Execute( ClipperLib::ctIntersection, solutionTree, ClipperLib::pftNonZero, ClipperLib::pftNonZero ) )
			{
				GenerateFromClipResults( solutionTree, *( *it ), 1.0f, runtimedropSectors );

				for ( size_t i = 0; i < runtimedropSectors.size(); ++i )
				{
					const Vector3f dropPos = runtimedropSectors[ i ].CalculateCenter();
...
void PathPlannerNavMesh::RuntimeNavSector::RebuildObstacles( PathPlannerNavMesh * navmesh )
{
	...
	if ( mObstacles.size() > 0 )
	{
		...
		ClipperLib::Paths obstaclePolys;
		...
		for ( size_t i = 0; i < mObstacles.size(); ++i )
		{
			if ( mObstacles[ i ]->IsActive() )
			{
				...
				obstaclePolys.push_back( ClipperLib::Path() );
				Vector3ListToClipper( obstaclePolys.back(), mObstacles[ i ]->mPoly );
			}
		}
		...
		if ( obstaclePolys.size() > 0 )
		{
			ClipperLib::Clipper clip;
			clip.AddPaths( subjectPolys, ClipperLib::ptSubject );
			clip.AddPaths( obstaclePolys, ClipperLib::ptClip );

			// first we get the sectors MINUS all the obstacles
			ClipperLib::PolyTree minusObstacles;
			if ( clip.Execute( ClipperLib::ctDifference, minusObstacles, ClipperLib::pftNonZero, ClipperLib::pftNonZero ) )
				GenerateFromClipResults( minusObstacles, *this, 1.0f, newSectors );
			...
			{
				ClipperLib::PolyTree obstacleIntersection;
				if ( clip.Execute( ClipperLib::ctIntersection, obstacleIntersection, ClipperLib::pftNonZero, ClipperLib::pftNonZero ) )
					GenerateFromClipResults( obstacleIntersection, *this, FLT_MAX, newSectors );
			}
			else
			{
				...
				while ( o < activeObstacles.size() )
				{
					...
					while ( o < activeObstacles.size() )
					{
						...
						{
							clip.AddPath( *( obstaclePolys.begin() + o ), ClipperLib::ptClip );
						}
						else
							break;
					...
					if ( clip.Execute( ClipperLib::ctIntersection, solutionTree, ClipperLib::pftNonZero, ClipperLib::pftNonZero ) )
					{
						...
						{
							GenerateFromClipResults( solutionTree, *this, setCost, newSectors );
						}
						else
						{
							ClipperLib::Paths obstacleSetPolys;
							ClipperLib::ClosedPathsFromPolyTree( solutionTree, obstacleSetPolys );
							
							clip.Clear();
							clip.AddPaths( obstacleSetPolys, ClipperLib::ptSubject );
							clip.AddPaths( obstaclePolys.begin() + o, obstaclePolys.end(), ClipperLib::ptClip );

							ClipperLib::PolyTree withoutNextSet;
							if ( clip.Execute( ClipperLib::ctDifference, withoutNextSet, ClipperLib::pftNonZero, ClipperLib::pftNonZero ) )
								GenerateFromClipResults( withoutNextSet, *this, setCost, newSectors );
						}
					}
				}
			}
	...
	if ( mSubSectors.size() > 0 )
	{
		...
		for ( size_t i = 0; i < mSubSectors.size(); ++i )
		{
			...
			Material mat;
			mat.mName = va( "subsector_%d", mIndex ).c_str();
			mat.mUser = &ns;
	...
	{
		Material mat;
		mat.mName = va( "moversector_%d", mIndex ).c_str();
		mat.mUser = this;
		navmesh->mRuntimeSectorCollision[ mIndex ] = new CollisionModel();
	...
	NavSectorBase & dest ) const
{
		...
		mirror == NavmeshIO::Sector_MirrorDir_ReflectZ )
	{
		...
		for ( uint32_t m = 0; m < dest.mPoly.size(); ++m )
		{
...
//
//	if ( solid )
//		RenderBuffer::AddPathFilled( polyList, color );
//	else
//		RenderBuffer::AddPathsilouette( polyList, color );
//
//	if ( vertSize > 0.0 )
//	{
...
//}
//
//static void DrawPoly( bool solid, const PathPlannerNavMesh::RuntimeNavSector & ns, const Polygon & poly, float offset, obColor color, float dur, float vertSize )
//{
//	Vector3List polyList;
...
{
	const uint8_t alpha = mSectorData->ledge() ? 50 : 100;
	...
	while ( fields0.size() > 0 )
	{
		...
		if ( !field->is_repeated() && !field->options().GetExtension( NavmeshIO::hidden ) )
		{
			...
			else
			{
				std::string s = va( "%s - %s",
					fullVarPath.c_str(),
					GetFieldString( msg0, field ).c_str() ).c_str();
	...
	while ( fields1.size() > 0 )
	{
		...
		if ( !field->is_repeated() && !field->options().GetExtension( NavmeshIO::hidden ) )
		{
			...
			else
			{
				std::string s = va( "%s - %s",
					fullVarPath.c_str(),
					GetFieldString( msg1, field ).c_str() ).c_str();
				s += " <m> ";
...
{
	rmt_ScopedCPUSample( PathPlannerNavMesh );
	...
	if ( mFlags.mViewMode > 0 && Utils::GetLocalEntity().IsValid() )
	{
		//////////////////////////////////////////////////////////////////////////
		...
		for ( size_t i = 0; i < mRuntimeSectors.size(); ++i )
		{
			...
			if ( mFlags.mViewFlags > 1 )
			{
				...
				for ( size_t sr = 0; sr < sectorRefs.size(); ++sr )
				{
					const RuntimeNavSector * sec = sectorRefs[ sr ];
...
bool PathPlannerNavMesh::Load( const std::string &_mapname, bool _dl )
{
	...
	try
	{
		...
		for ( int s = 0; s < nm.sectors_size(); ++s )
		{
			vertices.resize( 0 );

			const NavmeshIO::Sector & sec = nm.sectors( s );

			NavSector sector;
			sector.mMirror = sec.sectormirrored();
			sector.mSectorData = sec.sectordata();
			sector.mSectorDataMirrored = sec.sectordatamirrored();

			ClearDefaultedValues( sector.mSectorData );
			ClearDefaultedValues( sector.mSectorDataMirrored );

			for ( int v = 0; v < sec.vertices_size(); ++v )
			{
				const NavmeshIO::SectorVert & vert = sec.vertices( v );
...
bool PathPlannerNavMesh::Save( const std::string &_mapname )
{
	...
	for ( uint32_t s = 0; s < mNavSectors.size(); ++s )
	{
		...
		*sectorOut->mutable_sectordatamirrored() = ns.mSectorDataMirrored;
		...
		for ( uint32_t v = 0; v < ns.GetNumVerts(); ++v )
		{
...
void PathPlannerNavMesh::InitSectors()
{
	...
	for ( size_t s = 0; s < mNavSectors.size(); ++s )
	{
		RuntimeNavSector runtimeSector( allSectors.size(), &mNavSectors[ s ], &mNavSectors[ s ].mSectorData );
		runtimeSector.Init( mNavSectors[ s ].GetVerts() );
		runtimeSector.mLocalPoly.resize( 0 );

		for ( int o = 0; o < runtimeSector.mSectorData->localoffsets_size(); ++o )
		{
			const NavmeshIO::SectorVert & v = runtimeSector.mSectorData->localoffsets( o );
	...
	for ( size_t i = 0; i < mRuntimeSectors.size(); ++i )
	{
		...
		Material mat;
		mat.mName = va( "sector_%d", i ).c_str();
		mat.mUser = &ns;
		...
		if ( ns.mSectorData->ledge() || ns.mSectorData->inwater() )
		{
			...
			for ( size_t j = 0; j < mRuntimeSectors.size(); ++j )
			{
				...
				subjectPolys.push_back( ClipperLib::Path() );
				Vector3ListToClipper( subjectPolys.back(), ns.GetVerts() );
				obstaclePolys.push_back( ClipperLib::Path() );
				Vector3ListToClipper( obstaclePolys.back(), other.GetVerts() );
				...
				ClipperLib::PolyTree unionSolution;
				if ( clip.Execute( ClipperLib::ctUnion, unionSolution, ClipperLib::pftNonZero, ClipperLib::pftNonZero ) )
					GenerateFromClipResults( unionSolution, ns, 1.0f, dropSectors );
		...
		if ( mFlags.mDisableObstacles == 0 )
		{
			for ( size_t o = 0; o < runtimeSector.mObstacles.size(); ++o )
			{
				...
				if ( !EngineFuncs::EntityVelocity( obs->mEntity, vel ) ||
					!IGame::GetEntityInfo( obs->mEntity, entInfo ) )
				{
					obs->mExpireTime = 0;
				}
				else
		...
		for ( size_t i = newObstacleStart; i < newObstacleEnd; ++i )
		{
			...
			while ( !openList.empty() )
			{
				...
				*sectorObstacle = *obs;
				...
				//for ( size_t p = 0; p < node->mPortals.size(); ++p )
				//{
				//	const NavPortal & portal = node->mPortals[ p ];
...
{
	Vector3f ray = from - to;
	const float maxDistance = ray.Normalize();

	CollisionModel * closestMdl = NULL;

	RayResult hitResult;
	if ( mSectorCollision.CollideRay( hitResult, ModelTransform::sIdentity, from, ray, maxDistance ) )
	{
		closestMdl = &mSectorCollision;
	}
	...
	for ( size_t i = 0; i < mRuntimeSectorCollision.size(); ++i )
	{
		...
		{
			RayResult subResult;
			if ( mRuntimeSectorCollision[ i ]->CollideRay( subResult, ModelTransform::sIdentity, from, ray, maxDistance ) )
			{
				if ( subResult.mHitDistance < hitResult.mHitDistance )
				{
	...
	if ( closestMdl )
	{
		...
		closestMdl->GetTriangle( hitResult.mHitTriangle, hitResult.mHitNode->mTransform, v, activeContentFlags, activeSurfaceFlags, materialIndex );

		const Material & mat = closestMdl->GetMaterial( materialIndex );
		return NavCollision( hitResult.mHitPos, hitResult.mHitNormal, static_cast<RuntimeNavSector*>( mat.mUser ) );
	}
	else
		return NavCollision();
}
...
void PathPlannerNavMesh::SectorsFromAttribs( RuntimeSectorRefs & sectorsOut, const Box3f & obb, const Indices & tris, const bool staticOnly )
{
	for( size_t tri = 0; tri <= tris.size(); ++tri )
	{
		Material material;
		if ( mSectorCollision.GetTriangleMaterial( tri, material ) )
		{
			RuntimeNavSector * ns = static_cast<RuntimeNavSector*>( material.mUser );
			...
			else
			{
				...
				ModelTransform subSectorXform( ns->mSectorWorldTrans, ns->mSectorWorldRot );

				Indices subSectorTris;
				mRuntimeSectorCollision[ ns->mIndex ]->CollideOBB( subSectorXform, obb, subSectorTris );

				for ( size_t subtri = 0; subtri <= subSectorTris.size(); ++subtri )
				{
					Material subMtrl;
					if ( mRuntimeSectorCollision[ ns->mIndex ]->GetTriangleMaterial( subtri, subMtrl ) )
					{
						sectorsOut.push_back( ns );
					}
				}
			}
		}
...
//int PathPlannerNavMesh::FindRuntimeSectors( const AABB & aabb, RuntimeSectorRefs & sectorsOut, const bool staticOnly )
//{
//	sectorsOut.resize( 0 );
//
//	Indices tris;
...
size_t PathPlannerNavMesh::FindRuntimeSectors( const Box3f & obb, RuntimeSectorRefs & sectorsOut, const bool staticOnly )
{
	sectorsOut.resize( 0 );

	Indices tris;
	mSectorCollision.CollideOBB( ModelTransform::sIdentity, obb, tris );
	SectorsFromAttribs( sectorsOut, obb, tris, staticOnly );
	return sectorsOut.size();
}
...
{
	if ( ei.mEntInfo.mFlags.CheckFlag( ENT_CAT_OBSTACLE ) )
	{
...
void PathPlannerNavMesh::EntityDeleted( const EntityInstance &ei )
{
	...
	{
		if ( mPendingObstacles[ i ]->mEntity == ei.mEntity )
		{

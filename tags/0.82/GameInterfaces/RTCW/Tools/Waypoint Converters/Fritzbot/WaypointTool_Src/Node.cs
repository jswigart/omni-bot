...
namespace WaypointTool
{
	public class Node : NavigationElementBase
	{
		...
		{
            // change these to test the node change
            int startPos = 11; // bytes 1 and 0 (set to 2 to change 2,1)
            int posValue = 100; // value (set to a val with enough difference so change should be visible)

            // UNCOMMENT to write the change
            //UpdateInt(posValue, startPos, ref _bytes);

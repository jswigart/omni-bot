	...
	public class NavigationReader
	{
		...
		public void Read(string fileName) 
		{
			...
			try 
			{
                ...
                // read header in so we can write it back out
                ReadSubBytes(readBytes, _header, 0);
				...
				if (_numberOfNodes > 0) 
				{
					for (int x = 0; x < _numberOfNodes; x++) 
					{
                        ...
                        MemoryStream memStream = new MemoryStream(nodeBytes);
                        BinaryReader binReader = new BinaryReader(memStream);

                        float xCoord = binReader.ReadSingle();
                        float yCoord = binReader.ReadSingle();
                        float zCoord = binReader.ReadSingle();
                        ...
                        {
                            Console.WriteLine("Byte " + xx + " = " + (int)nodeBytes[xx]);
                        }
                        ...
                        for (int xx = 0; xx < 12; xx++)
                        {
                            ...
                            if (secondByte < 12)
                            {
                                Console.WriteLine("Byte " + firstByte + ", " + secondByte + " = " + GetInt(nodeBytes[firstByte], nodeBytes[secondByte]));
                            }
                        }
                        ...
                        node.PositionX = xCoord;
                        node.PositionY = yCoord;
                        node.PositionZ = zCoord;

...
// RAVEN END
	...
	omnibot_interface::Bot_Event_FireWeapon(owner->entityNumber, 
			obUtilGetWeaponId(weaponDef->GetName()), proj);
}
...
void rvWeapon::Hitscan( const idDict& dict, const idVec3& muzzleOrigin, const idMat3& muzzleAxis, int num_hitscans, float spread, float power ) {
	...
	omnibot_interface::Bot_Event_FireWeapon(owner->entityNumber, obUtilGetWeaponId(weaponDef->GetName()), NULL);
}

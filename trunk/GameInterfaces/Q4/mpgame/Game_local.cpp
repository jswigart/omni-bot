...
TIME_THIS_SCOPE("idGameLocal::RunFrame - gameDebug.BeginFrame()");
		...
		// Sounds like something we don't want the bots to update in.
		if(!inCinematic)
		{
...
//  Omni-Bot
	if ( g_showSpawns.GetBool() ) {
		...
		viewBounds.ExpandSelf( 1024.0f );

		for( ent = spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next() ) {
			if( ent->IsType ( idPlayerStart::GetClassType() ) ) {
				ent->DrawDebugEntityInfo(&viewBounds, &viewTextBounds);
			}
		}
	}

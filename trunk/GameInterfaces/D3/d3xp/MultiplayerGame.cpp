...
void idMultiplayerGame::NewState( gameState_t news, idPlayer *player ) {
	...
	switch( news ) {
		case GAMEON: {
			...
			// Omni-bot - Tell the bot to start, so it can get the goals and stuff.
			//omnibot_interface::Bot_Event_StartGame();
...
void idMultiplayerGame::ProcessChatMessage( int clientNum, bool team, const char *name, const char *text, const char *sound ) {
	...
	if(send_to==2)
	{
		omnibot_interface::Notify_TeamChatMsg(p,text);
	} 
	else if(send_to==0)
	{
		omnibot_interface::Notify_ChatMsg(p,text);
	}
	//////////////////////////////////////////////////////////////////////////

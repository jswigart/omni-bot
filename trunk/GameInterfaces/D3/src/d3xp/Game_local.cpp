...
/*
===========
checkFreeSlot - return the next free slot num for bots or kick a bot in the desired slot for a connecting human
============
*/
int checkFreeSlot(bool isBot, int num)
{
	if (isBot)
	{
	...
	// kick a bot occupying connecting humans spot
	if (gameLocal.entities[num])
		gameLocal.ServerClientDisconnect(num);
...
void idGameLocal::LocalMapRestart( ) {
	...
	omnibot_interface::Bot_RestartGame();
}
...
void idGameLocal::InitFromNewMap( const char *mapName, idRenderWorld *renderWorld, idSoundWorld *soundWorld, bool isServer, bool isClient, int randseed ) {
	...
	// Omni-bot - Added InitBotInterface
	if(gameLocal.isServer)
	{
		if(!omnibot_interface::InitBotInterface())
			Error( "Unable to Initialize Bot Interface" );
	}
}
...
void idGameLocal::MapShutdown( void ) {
	// Omni-bot - Added ShutdownBotInterface
...
gameReturn_t idGameLocal::RunFrame( const usercmd_t *clientCmds ) {
	...
	if ( !isMultiplayer && g_stopTime.GetBool() ) {
		...
		// Omni-bot - Added UpdateBotInterface block
		if(!inCinematic)
		{
	...
	if ( g_showSpawns.GetBool() ) {
		...
		for( ent = spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next() ) {
			if( ent->IsType ( idPlayerStart::Type ) ) {
				const idBounds &entBounds = ent->GetPhysics()->GetAbsBounds();
				gameRenderWorld->DebugBounds( colorGreen, entBounds.Expand( 16.0f ) );
				if ( viewTextBounds.IntersectsBounds( entBounds ) ) {
					gameRenderWorld->DrawText( ent->name.c_str(), entBounds.GetCenter(), 0.1f, colorWhite, axis, 1 );
					gameRenderWorld->DrawText( va( "#%d", ent->entityNumber ), entBounds.GetCenter() + up, 0.1f, colorWhite, axis, 1 );
				}
...
void idGameLocal::RegisterEntity( idEntity *ent ) {
	...
	omnibot_interface::Bot_Queue_EntityCreated(ent);
}
...
void idGameLocal::UnregisterEntity( idEntity *ent ) {
	...
	omnibot_interface::Bot_Event_EntityDeleted(ent);

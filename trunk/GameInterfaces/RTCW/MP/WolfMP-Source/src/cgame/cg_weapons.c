...
#define BARREL_SMOKE_TIME 1000
	...
	//S4NDM4NN
	if(!cg_muzzleFlash.integer){ //if they have muzzleflash off dont show it
	flash.hModel = 0;
	}
	...
	if ( ps || cg.renderingThirdPerson || !isPlayer ) {
		if(weaponNum == WP_STEN || weaponNum == WP_VENOM ) {
			if ( cg.time - cent->muzzleFlashTime < 100 && cg_muzzleFlash.integer)
...
//			cg.zoomTime		= cg.time;
	...
	if(cg.zoomval > zoomTable[zoomindex][ZOOM_OUT])	cg.zoomval = zoomTable[zoomindex][ZOOM_OUT];
	if(cg.zoomval < zoomTable[zoomindex][ZOOM_IN])	cg.zoomval = zoomTable[zoomindex][ZOOM_IN];

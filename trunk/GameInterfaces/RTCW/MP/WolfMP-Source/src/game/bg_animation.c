...
int BG_AnimScriptAnimation( playerState_t *ps, aistateEnum_t state, scriptAnimMoveTypes_t movetype, qboolean isContinue )
{
...
#ifdef DBGANIMS
	if (ps->clientNum) Com_Printf( "script anim: cl %i, mt %s, ", ps->clientNum, animMoveTypesStr[movetype] );
#endif
	...
	if (!scriptItem) {
#ifdef DBGANIMS
		if (ps->clientNum) Com_Printf( "no valid conditions\n" );
#endif
		return -1;
	}
...
#ifdef DBGANIMS
	if (scriptCommand->bodyPart[0])
		if (ps->clientNum) Com_Printf( "anim0 (%s): %s", animBodyPartsStr[scriptCommand->bodyPart[0]].string, modelInfo->animations[scriptCommand->animIndex[0]].name );
	if (scriptCommand->bodyPart[1])
		if (ps->clientNum) Com_Printf( "anim1 (%s): %s", animBodyPartsStr[scriptCommand->bodyPart[1]].string, modelInfo->animations[scriptCommand->animIndex[1]].name );
	if (ps->clientNum) Com_Printf("\n");
#endif

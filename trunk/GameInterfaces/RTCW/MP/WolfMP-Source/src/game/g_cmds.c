...
int ClientNumbersFromString( char *s, int *plist) {
	...
	*plist = -1;
	...
	for(i=0; i < level.maxclients; i++) {
		...
		SanitizeString(p->pers.netname, n2);
		m = strstr(n2, s2);
		if(m != NULL) {
			*plist++ = i;
			found++;
...
void Cmd_GameCommand_f( gentity_t *ent ) {
	...
	if ( player < 0 || player > level.numConnectedClients ) {
		return;
	}
...
void Cmd_Stats_f(gentity_t *ent)
{
	...
	strcat(message, va("^3Goombas: ^7%d \n", client->pers.goomba));
...
Set PERS_HWEAPON_USE to test detection of invalid setting

==================
*/
void Cmd_TestMG_f (gentity_t *ent)
{
	gclient_t *p;
	int i;
	...
	{
		p = &level.clients[i];
		p->ps.persistant[PERS_HWEAPON_USE] = 1;
	}
}
...
//		Cmd_SetCameraOrigin_f( ent );
	...
	else if (Q_stricmp (cmd, "testmg") == 0)	
		Cmd_TestMG_f( ent);
	else

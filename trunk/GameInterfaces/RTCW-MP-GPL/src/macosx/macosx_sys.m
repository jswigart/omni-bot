...
void *Sys_LoadDll( const char *name, int( **entryPoint ) ( int, ... ),
	...
	NSString *bundlePath, *libraryPath, *installationPath;
	const char *path;
...
//    libraryPath = [NSString stringWithFormat: @"%@/Contents/MacOS/%s", bundlePath, name];
	installationPath = [[NSUserDefaults standardUserDefaults] objectForKey:@ "RetailInstallationPath"];
	if ( !installationPath ) {
	...
	libraryPath = [NSString stringWithFormat: @ "%@/%s.bundle/Contents/MacOS/%s", installationPath, name, name];
	if ( !libraryPath ) {
		return NULL;
	}
...
qboolean        Sys_CheckCD( void ) {
	...
	while ( mountCount-- ) {
		...
		lastComponent++;
		if ( ( strcasecmp( lastComponent, "WolfensteinMP" ) != 0 ) ) {
			continue;
		}

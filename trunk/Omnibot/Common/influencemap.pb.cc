...
const Vec3& Vec3::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_influencemap_2eproto();  return *default_instance_;
}
...
bool SpanColumn::IsInitialized() const {
  ...
  for (int i = 0; i < spans_size(); i++) {
    if (!this->spans(i).IsInitialized()) return false;
  }
  return true;
}

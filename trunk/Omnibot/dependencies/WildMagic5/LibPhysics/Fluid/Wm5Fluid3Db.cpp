    ...
    for (k = 1; k < mKMax; ++k)
    {
        for (j = 1; j < mJMax; ++j)
        {
            for (i = 1; i < mIMax; ++i)
            {
                ...
                int k0 = (int)Math<Real>::Floor(kPrevious);
                int k1 = k0 + 1;
                ...
                Real denXX =
                    mDensity0[k][j][i+1] - twoDen00 + mDensity0[k][j][i-1];
                Real denYY =
                    mDensity0[k][j+1][i] - twoDen00 + mDensity0[k][j-1][i];
                Real denZZ =
                    mDensity0[k+1][j][i] - twoDen00 + mDensity0[k-1][j][i];
                Real denSource = SourceDensity(mTime, mX[i], mY[j], mZ[k],
                    i, j, k);

                mDensity1[k][j][i] = denAdvect + ((Real)2)*(
                    mDenLambdaX*denXX + mDenLambdaY*denYY +
                    mDenLambdaZ*denZZ + mDt*denSource);
                ...
                Vector3<Real> twoVel00 = ((Real)2)*mVelocity0[k][j][i];
                Vector3<Real> velXX =
                    mVelocity0[k][j][i+1] - twoVel00 + mVelocity0[k][j][i-1];
                Vector3<Real> velYY =
                    mVelocity0[k][j+1][i] - twoVel00 + mVelocity0[k][j-1][i];
                Vector3<Real> velZZ =
                    mVelocity0[k+1][j][i] - twoVel00 + mVelocity0[k-1][j][i];
                Vector3<Real> velSource = SourceVelocity(mTime, mX[i], mY[j],
                    mZ[k], i, j, k);

                mVelocity1[k][j][i] = velAdvect + ((Real)2)*(
                    mVelLambdaX*velXX + mVelLambdaY*velYY +
                    mVelLambdaX*velZZ + mDt*velSource);
            }
        }
    }
...
//----------------------------------------------------------------------------
template <typename Real>
void Fluid3Db<Real>::DirichletBoundaryZero (Real*** data)
{
    int i, j, k;
...
//----------------------------------------------------------------------------
template <typename Real>
void Fluid3Db<Real>::NeumannBoundaryZero (Real*** data)
{
    int i, j, k;
...
//----------------------------------------------------------------------------
template <typename Real>
void Fluid3Db<Real>::UpdateDensityBoundary ()
{
...
//----------------------------------------------------------------------------
template <typename Real>
void Fluid3Db<Real>::SwapDensityBuffers ()
{
    Real*** save = mDensity0;
    mDensity0 = mDensity1;
    mDensity1 = save;
...
//----------------------------------------------------------------------------
template <typename Real>
void Fluid3Db<Real>::SwapVelocityBuffers ()
{

...
HORIZONTAL : (double)(pt2.X - pt1.X) / (pt2.Y - pt1.Y);
	...
	bool ClipperBase::AddPolygons( const Polygons::const_iterator itFirst, Polygons::const_iterator itEnd, PolyType polyType )
	{
		bool result = false;
		for ( Polygons::const_iterator it = itFirst;
			it != itEnd;
			++it )
		{
			if (AddPolygon(*it, polyType)) result = true;
		}
		return result;
	}
	...
	bool Clipper::GetPolyOut( int i, Polygon &poly, bool & isHole ) const
	{
		poly.resize( 0 );
		if (m_PolyOuts[i]->pts)
		{
			isHole = m_PolyOuts[i]->isHole;

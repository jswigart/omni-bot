...
void idGameLocal::ServerClientBegin( int clientNum ) {
	...
	// see if a bot needs kicked first
	clientNum = checkFreeSlot(false, clientNum);
	...
	// Omni-bot - Added Bot_Event_ClientConnected
	omnibot_interface::Bot_Event_ClientConnected(clientNum, omnibot_interface::IsBot( clientNum ));
}
...
void idGameLocal::ServerClientDisconnect( int clientNum ) {
	...
	// Omni-bot - Tell Omni-bot that he's been disconnected.
	omnibot_interface::Bot_Event_ClientDisConnected(clientNum);
...
void idGameLocal::ServerWriteSnapshot( int clientNum, int sequence, idBitMsg &msg, byte *clientInPVS, int numPVSClients ) {
	...
	// Omni-bot - Added WriteUserCmdsToSnapshot
	...
	omnibot_interface::WriteUserCmdsToSnapshot( msg );
...
void idGameLocal::ClientReadSnapshot( int clientNum, int sequence, const int gameFrame, const int gameTime, const int dupeUsercmds, const int aheadOfServer, const idBitMsg &msg ) {
	...
	// Omni-bot - Added ReadUserCmdsFromSnapshot
	...
	omnibot_interface::ReadUserCmdsFromSnapshot( msg );

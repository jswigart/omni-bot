...
public:
	...
	flagState_t		GetFlagState( int flag ) const;
	int				GetFlagCarrier( int flag ) const;
...
ID_INLINE flagState_t rvCTFGameState::GetFlagState( int flag ) const {
	assert( flag >= 0 && flag < TEAM_MAX );

	return flagStatus[ flag ].state;
}

ID_INLINE int rvCTFGameState::GetFlagCarrier( int flag ) const {
	assert( flag >= 0 && flag < TEAM_MAX );

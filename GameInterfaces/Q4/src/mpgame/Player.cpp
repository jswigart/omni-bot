...
void idInventory::RestoreInventory( idPlayer *owner, const idDict &dict ) {
	...
	// Omni-bot - Notify the bot we stole all his stuff
	if(owner)
	{		
		omnibot_interface::Bot_Event_ResetWeapons(owner->entityNumber);
	}
...
// RAVEN END
	...
	// Omni-bot - Notify the bot he got a new weapon.
			if(owner)
			{
				omnibot_interface::Bot_Event_AddWeapon(owner->entityNumber, obUtilGetWeaponId(weaponName));
			}
	...
	if( gameLocal.isMultiplayer && gameLocal.mpGame.IsBuyingAllowedInTheCurrentGameMode() )
	{
		...
		for( int weaponIndex = 0; weaponIndex < MAX_WEAPONS; weaponIndex++ )
		{
			if( inventory.weapons & ( 1 << weaponIndex ) )
			{
				...
				// Omni-bot - Notify the bot he got a new weapon.
				const char *weap = spawnArgs.GetString( va( "def_weapon%d", weaponIndex ) );
				if(weap && weap[0])
					omnibot_interface::Bot_Event_AddWeapon(entityNumber, obUtilGetWeaponId(weap));
			}
		}
	...
	if ( gameLocal.mpGame.IsBuyingAllowedInTheCurrentGameMode() ) {
		...
		if( !idStr::Icmpn( itemname, "weapon_", 7 ) ) {
			...
			if( weaponIndex >= 0 && weaponIndex < MAX_WEAPONS )
			{
				...
				// Omni-bot - Notify the bot he got a new weapon.
				omnibot_interface::Bot_Event_AddWeapon(entityNumber, obUtilGetWeaponId(itemname));
			}
		}
...
// RAVEN END
	...
	if ( damage > 0 ) {
		...
		if ( health <= 0 ) {
			...
			//////////////////////////////////////////////////////////////////////////
			// Omni-bot - Notify the bot he died
			//const idDict *damageDict = gameLocal.FindEntityDefDict( damageDefName, false );
			omnibot_interface::Bot_Event_Death(entityNumber, attacker, damageDefName);
			// Omni-bot - Notify the bot he killed someone.
			if(attacker)
			{
				//const idDict *damageDict = gameLocal.FindEntityDefDict( damageDefName, false );
				//obUserData udIn(damageDefName ? damageDefName : "unknown");
				omnibot_interface::Bot_Event_KilledSomeone(attacker->entityNumber, this, damageDefName);
			}
			//////////////////////////////////////////////////////////////////////////
		} else {
			// force a blink
	...
	// Omni-bot - Notify the bot he took damage.
	omnibot_interface::Bot_Event_TakeDamage(entityNumber, attacker);
}

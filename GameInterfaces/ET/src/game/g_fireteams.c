...
void G_AddClientToFireteam( int entityNum, int leaderNum ) {
	...
	for( i = 0; i < MAX_CLIENTS; i++ ) {
		...
		if(ft->joinOrder[i] == -1) {
			...
			// omnibot
			Bot_Event_JoinedFireTeam(&g_entities[entityNum],&g_entities[leaderNum],ft->ident);
			// end omnibot
...
void G_WarnFireTeamPlayer( int entityNum, int otherEntityNum ) {
	...
	// omnibot
	Bot_Event_FireTeam_Warn(&g_entities[entityNum], &g_entities[otherEntityNum], ft->ident);
	// end omnibot
}
...
void G_ProposeFireTeamPlayer( int entityNum, int otherEntityNum ) {
	...
	// omnibot
	Bot_Event_FireTeam_Proposal(leader,&g_entities[otherEntityNum],ft->ident);
	// end omnibot
}

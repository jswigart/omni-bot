...
static void CG_Obituary( entityState_t *ent ) {
	...
	if ( attacker != ENTITYNUM_WORLD ) {
		...
		case MOD_KNIFE2:
			...
			message2 = "'s knife";
			if ( cg_goat.integer && ( attacker == cg.snap->ps.clientNum || target == cg.snap->ps.clientNum ) ) {
...
void CG_Explodef( vec3_t origin, vec3_t dir, int mass, int type, qhandle_t sound, int forceLowGrav, qhandle_t shader ) {
	...
	for ( i = 0; i < POSSIBLE_PIECES; i++ ) {
		...
		for ( howmany = 0; howmany < pieces[i]; howmany++ ) {
			...
			case 3: // "gibs"
				goto pass; // cs: gibs aren't in mp
...
void CG_EntityEvent( centity_t *cent, vec3_t position ) {
	...
	case EV_STOPSTREAMINGSOUND:
		...
		// powerup events
		//
	case EV_POWERUP_NADEPACK:
		DEBUGNAME( "EV_POWERUP_NADEPACK" );
		trap_S_StartSound( es->pos.trBase, -1, CHAN_AUTO, cgs.media.powerupSound );
		break;

	case EV_POWERUP_SMOKEGRENADE:
		DEBUGNAME( "EV_POWERUP_SMOKEGRENADE" );
		trap_S_StartSound( es->pos.trBase, -1, CHAN_AUTO, cgs.media.powerupSound );
		break;

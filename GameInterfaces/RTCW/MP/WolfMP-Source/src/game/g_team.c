...
// cs: i doubt anyone will miss this. removing to save a bit of cpu 
/*	flag = NULL;
...
// jpw
	...
	if ( self->count == TEAM_RED )
	{
		G_Script_ScriptEvent( self, "trigger", "axis_capture" );
		if ( g_deathmatch.integer )
			Bot_Util_SendTrigger(self, NULL, va("dm_axis_%s_%s", flagAction, _GetEntityName(self)), flagAction);
		else

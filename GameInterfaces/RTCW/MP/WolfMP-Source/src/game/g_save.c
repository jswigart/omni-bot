...
void WriteField2 (fileHandle_t f, saveField_t *field, byte *base)
{
	...
	case F_STRING:
		...
		{
			len = strlen(*(char **)p) + 1;
			if (!trap_FS_Write (*(char **)p, len, f)) G_SaveWriteError();
		}
		break;
	case F_FUNCTION:
		if ( *(byte **)p )
		{
			...
			len = strlen(func->funcStr) + 1;
			if (!trap_FS_Write (func->funcStr, len, f)) G_SaveWriteError();
		}
		break;
...
void WriteClient(fileHandle_t f, gclient_t *cl)
{
	...
	// write the block
	if (!trap_FS_Write (&temp, sizeof(temp), f)) G_SaveWriteError();
...
void G_UpdatePlayTime(void)
{
	...
	if (!trap_FS_Write (&i, sizeof(i), of)) G_SaveWriteError();
	len -= sizeof(i);

	// read the mapname
	trap_FS_Read (mapname, MAX_QPATH, f);
	if (!trap_FS_Write (mapname, MAX_QPATH, of)) G_SaveWriteError();
	len -= MAX_QPATH;
	...
	while (len) {
		...
		if (!trap_FS_Write (mapname, i, of)) G_SaveWriteError();
	}

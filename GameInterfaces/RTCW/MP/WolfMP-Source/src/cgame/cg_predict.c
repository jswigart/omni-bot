...
CS: duplicated here since it's a pain in the ass any other way
*//////////////////////////////////////////////////
qboolean CG_PlayerNeedsAmmo()
{
	int nades, weapon, i;
	int numClips = 3;					
	...
	if (cg.snap->ps.ammoclip[BG_FindClipForWeapon(weapon)] < nades)
		return qtrue;
	...
	if (cg.snap->ps.ammo[BG_FindAmmoForWeapon(weapon)] < ammoTable[weapon].maxclip*4)
		return qtrue;
	
	if (cg.snap->ps.stats[STAT_PLAYER_CLASS] == PC_MEDIC)
	{
		if (cg.snap->ps.ammoclip[BG_FindClipForWeapon(WP_MEDIC_SYRINGE)] < 10)
			return qtrue;
	}

// CS: TODO: communicate this value from server info. not sure it's really worth it right now
...
//	}
	...
	for (i = 0; i < MAX_WEAPS_IN_BANK_MP; i++)
	{
		...
		if (COM_BitCheck(cg.snap->ps.weapons, weapon))	
		{
			...
			{
				if (cg.snap->ps.ammoclip[BG_FindAmmoForWeapon(weapon)] < ammoTable[weapon].maxclip)
					return qtrue;
			}
			
			if (cg.snap->ps.ammo[BG_FindAmmoForWeapon(weapon)] < ammoTable[weapon].maxclip*numClips)
				return qtrue;
		}
	}

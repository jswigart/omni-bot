...
vec4_t	alliesRectFill = { 0.5f, 0.25f, 0.25f, 0.25f };

//cs: right aligned overlays; fps, local time, etc
#define RAO_X 569
#define RAO_H 18
#define RAO_W 74
#define RAO_O 250 // offset from team overlay
...
static float CG_DrawTimer( float y ) {
	char		*s;
	int			w;
	int			mins, seconds, tens, lmins=0, lseconds=0, ltens=0;
	int			msec, limbo;
...
void CG_DrawDeathmatchScore() {
	...
	if ( cg_drawDmScore.integer )
	{
		...
		s = va("%d", cgs.scores1);
		wRed = CG_DrawStrlen(s);
		CG_DrawGiantStringColor(x,y,s,vecRed);

		s = va("%d", cgs.scores2);
		wBlue = CG_DrawStrlen(s);
		CG_DrawGiantStringColor(xx - ((int)(wBlue * GIANTCHAR_WIDTH)) + GIANTCHAR_WIDTH,y,s,vecBlue);
	}
}
...
void CG_DrawObjectiveIcons() {
	...
	int limbo, lmins=0, lseconds=0, ltens=0;
